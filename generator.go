...
func (g *generator) do() error {
	...
	for i := range g.structures {
		...
		if g.defs[g.structures[i].Name] != nil {
			...
			case *ast.ArrayType:
				stmt = jen.Id(idObjectID).Op("==").Nil().Op("||").Len(jen.Op("*").Id(idObjectID)).Op("==").Op("0")

			case *ast.StructType:
				stmt = jen.Id(idObjectID).Op("==").Nil().Op("||").Op("*").Id(idObjectID).Op("==").Id(objectID).Op("{}")

			case *ast.MapType:
				stmt = jen.Id(idObjectID).Op("==").Nil()

			default:
				fmt.Println(">> other structure: " + g.structures[i].Name.String())
				return nil
			}

			var err error

			g.buf.Comment("IsEmpty ...")
			g.buf.Func().Params(
				jen.Id(idObjectID).Op("*").Id(objectID),
			).Id("IsEmpty").Params().Bool().BlockFunc(func(grp *jen.Group) {
				// err = g.doOne(
				// 	grp,
